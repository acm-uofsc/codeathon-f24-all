...
for i in range(0, 41):
    ...
    try:
        worked = subprocess.run(f'python3 ./solutions/clean_sol.py < {input_file} > {output_file}', 
                                shell=True, check=True, capture_output=True, text=True)
    except subprocess.CalledProcessError as e:
        # Print the error and exit if the command fails
    ...
    try:
        temp_file = "temp.txt"
        worked = subprocess.run(f'python3 ./solutions/gpt_shunting_yard.py < {input_file} > {temp_file}', 
                                shell=True, check=True, capture_output=True, text=True)
        worked = subprocess.run(f'diff {output_file} {temp_file}', 
                                shell=True, check=True, capture_output=True, text=True)
